...
def make_plan(start_time, balloon, wind, horizon_time, destination, key, discount=0.99, vertical_weight=111):
    ...
    init_cost = cost_at(START_TIME, balloon, init_plan, wind, destination)
    ...
    def add_plan(i, key_bestplancost):
        ...
        cost = cost_at(start_time, balloon, plan, wind, destination, discount, vertical_weight)
...
@jax.jit
def cost_at(start_time, balloon, plan, wind, destination, discount=0.99, vertical_weight=111):
...
def gradient_at(start_time, balloon, plan, wind, destination, discount=0.99, vertical_weight=111):
    N = (len(plan)*WAYPOINT_TIME_STEP)//INTEGRATION_TIME_STEP
    def inner_run(i, time_balloon_cost):
        time, balloon, cost = time_balloon_cost
        cost += discount**(N-1-i)*((balloon.state[0]-destination[0])**2 + (balloon.state[1]-destination[1])**2 + (vertical_weight*(balloon.state[2]-destination[2])/111)**2) 
...
@jax.jit
def get_optimal_plan(start_time, balloon, plan, wind, destination, discount=0.99, vertical_weight=111):

    optimizer = optax.adam(learning_rate = 0.03)
    opt_state = optimizer.init(plan)
    grad = gradient_at(start_time, balloon, plan, wind, destination, discount, vertical_weight)
...
def unjitted_receeding_horizon_control(start_time, time_elapsed, balloon, observed_wind, true_wind, key, destination):
    ...
    while time < start_time + time_elapsed and jnp.any(next_plan>0):
        ...
        optimal_plan = get_optimal_plan(time, balloon, next_plan, observed_wind, destination)
        plan_to_follow = optimal_plan[:follow_time//WAYPOINT_TIME_STEP+1]
    ...
    optimal_plan = get_optimal_plan(time, balloon, next_plan, observed_wind, destination, discount=0.9)
    plan_to_follow = optimal_plan[:follow_time//WAYPOINT_TIME_STEP+1]
